'''Generate training set and test set files '''
...
def write_comps_ratings(f, user_id, ratings_list):
  ...
  for (rating1, rating2) in itertools.combinations(ratings_list, 2):
    if rating1[1] > rating2[1]:      
      line = line + "{0}:{1} ".format(rating1[0], rating2[0])
    if rating1[1] < rating2[1]:
      line = line + "{0}:{1} ".format(rating2[0], rating1[0])
  print(line, file=f)
...
def raw2listdata2(filename, output, n_train, n_test):
  n_users = 0
  n_items = 0
  n_ratings = 0
  triples_list = []
  ...
  g1 = open(output + '_train_'+str(n_train)+'.txt', 'w')
  g2 = open(output + '_test_'+str(n_train)+'.txt', 'w')
  g3 = open(output + '_train_comp_'+str(n_train)+'.txt', 'w')
  ...
  for u in xrange(1, n_users+1):
    ...
    if len(ratings_list) >= n_train + n_test:
      ...
      write_lsvm(g1, user_id, train)
      write_lsvm(g2, user_id, test)
      write_comps_ratings(g3, user_id, train)

  g1.close()
  g2.close()
  
  print("Lists generated for {0} users".format(user_id))
...
if __name__ == "__main__":
  ...
  raw2listdata2(args.input_file, args.output, args.n_train, args.n_test)
