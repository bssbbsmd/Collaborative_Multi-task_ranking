...
void Solver::initialize(Problem& prob, Model& model, init_option_t option) {
  ...
  switch(option) {
    ...
    srand(time(NULL)); 
    for(int i=0; i<n_users*model.rank; i++) model.U[i] = (double)rand() / (double)RAND_MAX / ((double)model.rank) /10;
    for(int i=0; i<n_items*model.rank; i++) model.V[i] = (double)rand() / (double)RAND_MAX / ((double)model.rank) /10;
    break;
