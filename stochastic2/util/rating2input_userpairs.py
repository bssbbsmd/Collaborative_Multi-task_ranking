...
def num2comp(filename, output, n_train, n_test):
  ...
  triples_list = []  # array linked list, n_users * rating_list(the items one use contains)

  f = open(filename, 'r')
  for line in f:
    (item_id, user_id, rating) = line.strip().split()
  ...
  for triple in train_triples:
  	...
  	if curr_iid != prev_iid:
  		if not rating_list:  # check if list is empty
  			continue
  		write_comps_ratings(g4, prev_iid, rating_list)
  		rating_list = []
